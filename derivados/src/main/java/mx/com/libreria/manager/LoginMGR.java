...
public class LoginMGR implements ContextConstant {
	...
	public boolean sendEmail(String from, String to, String subject, String messageBody, String[] attach) { 
		...
		try {
			...
			Logging.debug(LoginMGR.class, "Mesae");
			Session ses = ContextEmail.getSessionEmail(true, true);
			Logging.debug(LoginMGR.class, "Mesae  2222");
			mail.setSession(ses);
			mail.sendMail(from, to, subject, messageBody, attach);
		    flag = true;
		} catch (Exception e) {
			Logging.error(LoginMGR.class, "Errores: " + e.toString());
			flag = false;
		}
		return flag;
	}
	...
	public int dmlOperations(int operacion, Object obj) {
		...
		try { 
			...
			Logging.debug(LoginMGR.class, " dmlOperations " + e.toString());
			result = -1;
		}
	...
	public static void main(String[] args) { 
		...
		if (login.existUser(user)) { 
			Logging.debug(Logging.class, "Usuario ya existe");
		} else {
			login.registerUser(user);
			Logging.debug(Logging.class, "Usuario creado");
		}
